...
class GaussianMixtureModel:
    ...
    def update_E_log_det_Lambda(self) -> None:
        ...
        for i in range(self.D):
            ...
            self.E_log_det_Lambda += psi((self.nu - i)/2)
    ...
    def update_Phi(self) -> None:
        ...
        for m in range(self.M):
            ...
            self.Phi[m] = self.Lambda_0

            self.Phi[m] += self.N_barra[m]*self.S_barra[m]

            self.Phi[m] += self.N_barra[m]*self.tau_0/self.tau[m]*np.outer(self.X_barra[m] - self.mu_0, self.X_barra[m] - self.mu_0)
    ...
    def estimates_Z(self) -> None:
        ...
        self.Z = self.gamma.argmax(axis = 1)
